    ...
    <b-card-body>
      ...
      <div v-if="election.is_owned">
        <template v-if="election.state === 'CREATED'">
          ...
          <p><b-button v-on:click="openElection()" variant="warning">Open election</b-button></p>
        </template>
        <template v-else-if="election.state === 'OPENED'">
          <h4>Election is opened</h4>
          <p><b-button v-on:click="voteInElection()">Vote in this election!</b-button></p>
          <p><b-button v-on:click="closeElection()" variant="warning">Close election and count votes</b-button></p>
        </template>
        <template v-else-if="election.state === 'CLOSED'">
          <h4>Election is closed</h4>
          <p><b-button v-on:click="showElectionResults()">See election results</b-button></p>
        </template>
      </div>
      <div v-else>
        <template v-if="election.state === 'CREATED'">
        ...
        <template v-else-if="election.state === 'CLOSED'">
          ...
          <p>
            <b-button
              v-on:click="showElectionResults()"
              variant="success"
